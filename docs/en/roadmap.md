...
* **Optional per-repo scope for mem0** — the hosted platform carries no bridge-side scope; layering there needs metadata filters (the cure\_memory arm's two-layer scoping — repo-bound vs general — has landed).
