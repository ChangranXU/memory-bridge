"""Host-side mem0 Platform backend for the automatic-extraction arm.
...
recording, periodic platform adds (mem0's hosted extraction stands in for a
local extraction lane — there is no local extraction client), transient recall
over the platform search, and the final dump. The event log and memory.json
artifact are the base's.
...
episode, platform adds as generation operations whose changes are the
platform's own ADD/UPDATE/DELETE receipts (``native_receipt`` evidence), and
recalls as search + main delivery. The memory lane carries no model traffic
(the extraction is hosted), so its annotate endpoint comes from the explicit
...
Scoping: mem0's store is hosted and persistent across run roots, so run
isolation comes from the effective user id. ``scope=run`` keeps the configured
user id for the whole run root (cross-instance recall); ``scope=instance``
appends the instance id. The driver mints the configured user_id from the
timestamped run-root name, so a fresh run root never recalls a previous run's
memories — and a stale attempt inside a run root is refused for the same
reason CURE refuses it (the rerun would recall the aborted attempt's
memories).
"""
...
# The final memory dump is diagnostic; one page far exceeds what a run root's
# episodes produce.
_FINAL_DUMP_PAGE_SIZE = 100
_ADAPTER_NAME = "mem0"
_IDENTITY_SCHEME = "mem0-platform-memory-v1"
...
class Mem0Backend(BaseMemoryBackend):
    ...
    def __init__(self, config: Mem0Config, instance_id: str, model_base_url: str = ""):
        ...
        self._client: Mem0PlatformClient | None = None
        self._pending: list[dict] = []  # recorded messages not yet flushed to the platform
    ...
    def _resolve_settings(self) -> dict:
        """Config value wins over $MEM0_API_KEY / $MEM0_BASE_URL; unavailable without a key."""
    ...
    def _make_platform_client(self, settings: dict) -> Mem0PlatformClient:
        """Isolated platform-client constructor (the test seam)."""
        return Mem0PlatformClient(api_key=settings["api_key"], base_url=settings["base_url"])

    def _startup(self, settings: dict) -> None:
        # Only the safe form is persisted: no userinfo/query/fragment — the
        # client below keeps the real URL (project rule 4).
        self._settings["api_base_url"] = sanitize_url(settings["base_url"])
        self._client = self._make_platform_client(settings)
        self._client.ping()  # fail fast on a bad key or endpoint
        # Unique per episode: stamped as the platform-side run_id so the
        # dashboard can attribute memories to one agent episode.
    ...
    def _start_event_extras(self) -> dict:
        return {"user_id": self.effective_user_id(), "api_base_url": self._settings["api_base_url"]}
    ...
    def _memory_ref(self, hit: dict) -> dict:
        """One platform memory as a portable native-stable MemoryRef.

        The platform id is stable across UPDATE versions, so the version id
    ...
    def _attribute_changes(self, operation, result, after):
        """Platform receipts -> the change series (evidence: native_receipt).

        The hosted store offers no before-image (no extra get_all per flush —
    ...
    def _generation_end_context(self, step, result, audit: dict) -> dict:
        """extensions.mem0 for the generation end: the platform event counts
    ...
    def _perform_extraction(self, step) -> None:
        """Flush the pending buffer as one platform add (the hosted extraction).

        The buffer is retained across failures and cleared only on success, so
        a failed batch is retried at the next boundary (the platform dedupes
        ...
        try:
            results = self._client.add(
                ...
                infer=self.config.infer,
                custom_instructions=self._extraction_guidelines() or None,
                poll_budget=self.config.poll_budget,
                poll_interval=self.config.poll_interval,
            )
        ...
        for item in results:
            ...
            if event == "ADD":
                ...
                # Not an extraction failure (the platform answered fine), but
    ...
    def _search(self) -> list:
        ...
        # With a host-side floor set, the platform's top_k cut would otherwise
        ...
        try:
            ...
            raise
        # A hit without a platform id is malformed and uncitable (the endpoint
    ...
    def _hit_origin(self, hit: dict) -> str | None:
        # The platform echoes the add-time run_id (the backend's per-episode
    ...
    def _hit_score(self, hit: dict) -> float | None:
        # The platform's combined multi-signal score (0-1), confirmed present
        # on search rows; an unusable one drops (never coerced) like in refs.
        if not isinstance(hit, dict):
            return None
    ...
    def _final_dump(self) -> list[dict]:
        if self._client is None:
            return []
        envelope = self._client.get_all(user_id=self.effective_user_id(), page_size=_FINAL_DUMP_PAGE_SIZE)
        final_memories = [
            {
                "id": item.get("id"),
                "memory": item.get("memory"),
                # The v3 get-all surface omits run_id; the same value rides its
                # session_id field — reading it keeps the dump's provenance.
                "run_id": item.get("session_id"),
